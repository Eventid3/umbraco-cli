...
This Umbraco CLI, is a tool designed for AI Agents. It's meant to be an alternative to the Umbraco MCP server, that is (hopefully) lighter in token usage.
...
```sh
go install github.com/Eventid3/umbraco-cli@latest
```

The binary will be installed as `umbraco-cli`. You may want to alias it:

```sh
alias umbraco=umbraco-cli
```

### From release binary

Download the latest release from the [GitHub releases page](https://github.com/Eventid3/umbraco-cli/releases) and place the binary somewhere on your `$PATH`:

```sh
# Example for Linux (amd64)
curl -L https://github.com/Eventid3/umbraco-cli/releases/latest/download/umbraco-linux-amd64 -o /usr/local/bin/umbraco
chmod +x /usr/local/bin/umbraco
```
